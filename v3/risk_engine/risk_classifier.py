"""
v3/risk_engine/risk_classifier.py
...
class RiskProfile:
    ...
    def to_dict(self) -> Dict[str, any]:
        return {
            ...
            'minimum_balance': float(self.minimum_balance_to_keep),
            'p50_safe': float(self.p50_safe_amount),
            'p90_safe': float(self.p90_safe_amount),
            'p50_headroom': float(self.p50_headroom),
            'p90_headroom': float(self.p90_headroom),
            'minimum_balance_p50': float(self.p50_min_closing),
            'minimum_balance_p90': float(self.p90_min_closing),
            'breach_p50': self.breach_under_p50,
            'breach_p90': self.breach_under_p90,
            'post_breach_p50': self.post_payment_breach_p50,
            'post_breach_p90': self.post_payment_breach_p90,
...
def classify_risk(
    ...
    # Compute top category stress contributors
    ...
    if diff_by_cat:
        ...
        from v3.risk_engine.stress_buffers import DEFAULT_P90_STRESS_FACTORS
        summary_parts = []
        for c in top_cats:
            factor = DEFAULT_P90_STRESS_FACTORS.get(c, Decimal('1.000'))
