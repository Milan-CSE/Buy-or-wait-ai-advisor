"""
forecast.py  (v2 - monthly aggregate approach)
Builds a 90-day daily cashflow ledger for a user.

APPROACH:
1. Explicitly scheduled future events (status=scheduled) are placed exactly on their dates
2. Pending debits are already reflected in starting_balance
3. For recurring projections:
   - INCOME: Group all income events by category. Detect cadence.
     Project at the detected cadence using median amount.
     Critical rule: if the latest salary says "Final" in description, DON'T project it.
   - EXPENSES: Group ALL expense events by (category). Compute average monthly total.
     Distribute the monthly total across the 90-day window proportionally.
     This avoids over-counting from multiple sub-streams within a category.
     Place as a single mid-month lump sum for simplicity.

4. The minimum balance across the entire window determines amount_safe_to_pay.
"""
    ...
    for item in state.recurring_debits:
        ...
        if spending_changes and item.event_id_for_change in spending_changes:
            ...
            if change is None:
                ...
                # If this is a daily burn item, the change is scaled per-day via minimum_allowed_amount
