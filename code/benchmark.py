...
def run_benchmark(use_daily_burn: bool = True, filter_unreliable_income: bool = True) -> None:
    """Run benchmark on all 25 sample requests."""
    burn_str = 'daily_burn' if use_daily_burn else 'v1_stepped'
    inc_str = 'reliable_only' if filter_unreliable_income else 'v1_all'
    print(f'Running benchmark on 25 sample requests (burn_mode={burn_str}, income_mode={inc_str})...\n')
    t0 = time.time()
    ...
    for sr in sample_requests:
        ...
        decision, diag, errors = process_request(
            req, dataset, fx, debug=False, use_daily_burn=use_daily_burn,
        ...
        failed_fields = [f for f, m in matches.items() if not m]
        field_str = f'  MISMATCH: {failed_fields}' if failed_fields else ''
        print(f'  {status} {req.request_id}: '
              f'{decision.recommended_payment_method} | {decision.affordability_status}{field_str}')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Run benchmark on sample requests')
    parser.add_argument('--burn-mode', choices=['daily_burn', 'v1_stepped'], default='daily_burn',
                        help='daily_burn (default) or v1_stepped (legacy)')
    parser.add_argument('--income-mode', choices=['reliable_only', 'v1_all'], default='reliable_only',
                        help='reliable_only (default) or v1_all (legacy all salary)')
