...
def parse_message_evidence(msg: Message) -> MessageEvidence:
    ...
    # ---- Pending payout unconfirmed ----
    if any(kw in text for kw in ['payout is still pending', 'balance isn\'t withdrawable',
                                  'payout shows as completed', 'pending', 'not withdrawable']):
        return MessageEvidence(
            message_id=msg.message_id,
            source_type=src,
